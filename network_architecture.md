...
2. At layer $\ell$ (where $\ell = 1,\dots,L$), we have $\mathbf{P}^{(\ell-1)} \in \mathbb{R}^{n_{\ell-1} \times c_{\ell-1}}$ describing the current set of nodes. We apply a “branching + transform” step:
...
1. **Forward direction (Auto-Encoder step)**  
   - Freeze/partial freeze the discriminator.  
   - Encode real $\mathbf{X}$ to $\mathbf{z}$, decode to $\widehat{\mathbf{X}}$.  
   - Update parameters minimizing $L_{\text{AE}}$.

2. **Reverse direction (GAN step)**  
   - Freeze/partial freeze the encoder.  
   - Generate $\widetilde{\mathbf{X}} = G(\mathbf{z})$ from noise.  
   - Discriminate real vs. fake.  
   - Update $D$/$G$ w.r.t. WGAN + optional NNME.

Since the encoder shares parameters with the discriminator, the AE step “guides” it to learn geometry features, while the GAN step forces the generator to create realistic shapes that fool the discriminator. This **collaborative** approach stabilizes learning with relatively few shapes.
